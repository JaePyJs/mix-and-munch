...
const ShoppingListPage: React.FC<{ mealPlan: MealPlanItem[] }> = ({ mealPlan }) => {
  ...
  const formatQuantity = (quantityMap: Map<string, number>): string => {
    return Array.from(quantityMap.entries())
      .map(([unit, qty]) => `${qty} ${unit === 'count' ? '' : unit}`.trim())
      .join(', ');
  };
  ...
  const handleCopy = () => {
    const listText = shoppingList
      .map(item => {
        ...
        return `- ${item.name}${quantityStr ? ` (${quantityStr})` : ''}`;
      })
      .join('\n');
  ...
  return (
    <div className="space-y-8">
      <div className="text-center p-6 bg-brand-gray-900 rounded-lg no-print">
      ...
      <div className="max-w-2xl mx-auto bg-brand-gray-900 rounded-lg p-6 printable-area">
        <div className="flex items-center justify-end gap-4 mb-6 no-print">
          <button onClick={handleCopy} className="bg-brand-gray-800 hover:bg-brand-gray-700 text-white font-medium py-2 px-4 rounded-lg transition-colors w-28">
            {copyButtonText}
          </button>
          <button onClick={handlePrint} className="bg-brand-lime text-brand-gray-900 font-bold py-2 px-4 rounded-lg transition-transform hover:scale-105">
            Print List
          </button>
        </div>
